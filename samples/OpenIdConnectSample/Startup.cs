            ...
            app.Run(async context =>
            {
                ...
                if (context.Request.Path.Equals("/signedout"))
                {
                    await WriteHtmlAsync(response, async res =>
                    {
                        ...
                        await res.WriteAsync("<a class=\"btn btn-default\" href=\"/\">Sign In</a>");
                ...
                if (context.Request.Path.Equals("/refresh"))
                {
                    ...
                    var handler = (AuthenticationHandler<OpenIdConnectOptions>)await authSchemes.GetHandlerAsync(context, OpenIdConnectDefaults.AuthenticationScheme);
                    var metadata = await handler.Options.ConfigurationManager.GetConfigurationAsync(context.RequestAborted);
                    ...
                    var tokenResponse = await handler.Options.Backchannel.PostAsync(metadata.TokenEndpoint, content, context.RequestAborted);
